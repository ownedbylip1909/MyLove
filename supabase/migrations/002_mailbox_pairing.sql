...
-- Remove policies from the first draft if it was already executed.
drop policy if exists "recipient creates pairing codes" on public.mailbox_pairing_codes;
drop policy if exists "recipient reads own pairing code metadata" on public.mailbox_pairing_codes;
drop policy if exists "senders insert letters" on public.letters;
drop policy if exists "senders update own mailbox letters" on public.letters;
drop policy if exists "recipient reads published letters" on public.letters;
